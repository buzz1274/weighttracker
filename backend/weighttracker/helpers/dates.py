...
class Dates:
    ...
    def __init__(self, date_to_modify: Optional[date] = None) -> None:
        if date_to_modify:
            self.date = date_to_modify
        else:
            self.date = date.today()

    def year_ago(self) -> Optional[date]:
    ...
    def month_ago(self) -> Optional[date]:
        ...
        try:
            ...
            return self.date.replace(
                ...
                day=calendar.monthrange(
                    year=self.date.year, month=self.date.month - 1
                )[1],
            )
        except AttributeError:
            return None

    def week_ago(self) -> Optional[date]:
